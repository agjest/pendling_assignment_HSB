...
pmat_long <- pmat_long |>
  group_by(aar, bo_kom) |>
  # ag_comment: Dropper round her. Vi angir heller antall digits vha. flextable
  # lengre nede i koden. Dette gir oss større fleksibilitet
...
set_flextable_defaults(
  # ag_comment: Setter big.mark til " " siden vi bruker "," som desimaltegn på norsk
  big.mark = " ",
  decimal.mark = ","
)
```
...
p2000_arb_percent <- pmat_long |>
  ...
  # velger de tre aktuelle variablene
  select(bo_kom, arb_kom, bo_percent) |> 
  # ag_comment: Vi trenger bo_percent. Litt forvirrende dette ;-). Det vi har definert som 
  # bo_percent er gitt at arbeidstakerene BOR i en av kommunene på Haugalandet. Hvor mange
  # prosent av disse JOBBER i de ulike kommunene
  pivot_wider(
    names_from = arb_kom, 
  ...
  select(
    # bo_kom , Haugesund, Karmøy, Tysvær, Sveio, Bokn, Vindafjord, Sauda, Etne, Andre
    # bruker vi all_of() funksjonen kan vi skrive
    all_of(ordKom)
    ) |>
  rename(
...
p2000_arb_percent |> 
  ...
  hrule(rule = "exact")  %>% 
  # Bruker neste for å angi at vi ønsker tre desimaler og suffix "%"
  # Her setter vi digits til 2, derfor vi fjernet round() for bo_percent
  # og arb_percent ovenfor
...
# Pendlematrise for Haugalandet 2000 - Arbeidskommune og boprosent
  ...
  # endrer første element i ordKom fra "bo_kom" til "arb_kom"
#ordKom[1] <- "arb_kom"
...
p2000_bo_percent <- pmat_long |>
  ...
  pivot_wider(
    names_from = arb_kom, 
    # ag_comment: arb_percent angir for hver arbeidsted-kommune, hvor mange BOR i de ulike kommunene
    #values_from = bo_percent
...
ag_comment: Nå kan v i lese svarene ut av de tre tabellene.
...
ag_comment: Nå har vi de tre tabellene for 2000.
Bruker samme kode for å lage tabeller for 2012.
Endrer bare filterert fra 2000 til 2012
...
ag_comment: Da har vi de tre tabellen for 2012 og kan lese svarene direkte ut av tabellene.
